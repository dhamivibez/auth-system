...
function SignupComponent() {
	...
	const [lastName, setLastName] = useState<string>(""); // Optional field
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			const response = await fetch(`${API}/auth/signup`, {
				...
				body: JSON.stringify({ firstName, lastName, username, email, password }), // Send all signup data
			});

			const data = await response.json(); // Parse the JSON response

			if (response.ok && data.success) {
				// Check for successful HTTP status and API success flag
				...
				// If signup fails, display the error message from the API or a generic one
		...
		<div className="min-h-screen flex items-center justify-center bg-gray-100 p-4 font-sans">
			<div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md border border-gray-200">
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					{/* First Name and Last Name Inputs (side-by-side on larger screens) */}
